...
class _ProfileScreenState extends State<ProfileScreen>
  ...
  Future<void> _loadSettings() async {
    ...
    // Update juga ke AttendanceProvider
  ...
  Future<void> _updateActiveMethod() async {
    ...
    // Simpan semua setting ke SharedPreferences
    ...
    // 🔥 TENTUKAN METODE AKTIF (Prioritas berurutan)
    String activeMethod = 'wajah'; // default
    ...
    print('✅ Active method saved: $activeMethod');
    print('✅ Active method name: $activeMethodName');
    print('✅ Active method icon: $activeMethodIcon');
  }
  ...
  void _onPresensiChanged(String method, bool value) {
    ...
    // Update metode aktif ke SharedPreferences
    _updateActiveMethod();
  }
    ...
    return Scaffold(
      ...
      body: FadeTransition(
        ...
        child: CustomScrollView(
          slivers: [
            // Header Premium dengan Avatar Besar
            SliverAppBar(
              expandedHeight: 320,
              pinned: true,
                              ...
                              Container(
                                ...
                                child: const CircleAvatar(
                                  ...
                                  child: Icon(Icons.person,
                                      size: 65, color: Color(0xFF2196F3)),
                                ),
                              ),
                              Positioned(
                                bottom: 0,
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            _buildInfoRow(Icons.person_outline, 'Nama Lengkap',
                                user?.name ?? 'Petugas', isDark),
                            const Divider(height: 0, indent: 20, endIndent: 20),
                            _buildInfoRow(Icons.phone_outlined, 'Nomor Telepon',
                                user?.phoneNumber ?? 'Belum diisi', isDark),
                            const Divider(height: 0, indent: 20, endIndent: 20),
                            _buildInfoRow(Icons.email_outlined, 'Email',
                                user?.email ?? 'petugas@gmail.com', isDark),
                            const Divider(height: 0, indent: 20, endIndent: 20),
                            if (isPetugas)
                              _buildInfoRow(Icons.badge_outlined, 'NIP',
                                  user?.nip ?? 'Belum diisi', isDark)
                            else
                              _buildInfoRow(Icons.class_outlined, 'Kelas',
                                  user?.kelas ?? 'Belum diisi', isDark),
                            const Divider(height: 0, indent: 20, endIndent: 20),
                            _buildInfoRow(Icons.work_outline, 'Jabatan',
                                isPetugas ? 'Petugas' : 'Siswa', isDark),
